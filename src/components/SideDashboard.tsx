...
export default function SideDashboard() {
  ...
  return (
    <>
      ...
      <div className="md:hidden mt-6">
        <div className="px-10 py-3 mb-6">
          <div className="flex items-center justify-between">
            <div>
              <p className="font-bold text-[25px]">Hi, User</p>
            ...
            <div className="flex items-center gap-3">
              ...
              <button onClick={shareLink}>
                ...
                {/* Notifications */} <button className="relative"> <svg xmlns="http://www.w3.org/2000/svg" className="w-7 h-7 text-[#331400]" viewBox="0 0 24 24" fill="fill" stroke="currentColor" strokeWidth={2} > <path strokeLinecap="round" strokeLinejoin="round" d="M15 17h5l-1.405-1.405A2.032 2.032 0 0118 14.158V11a6 6 0 10-12 0v3.159c0 .538-.214 1.055-.595 1.436L4 17h5" /> <path strokeLinecap="round" strokeLinejoin="round" d="M13.73 21a2 2 0 01-3.46 0" /> </svg> {/* Notification dot */} <span className="absolute -top-1 -right-1 w-2 h-2 bg-red-500 rounded-full" /> </button>
              {/* MORE */}
      ...
      {/*  DESKTOP QR SECTION  */} <div className="hidden md:block p-4 sm:p-6 space-y-4 text-gray-800 max-w-[400px] mx-auto"> <div className="flex items-center justify-between"> {/* <h1 className="font-semibold md: text-lg">David Osh</h1> */} <div className="flex items-center gap-6 mb-6"> <button onClick={() => setIsModalOpen(true)}> <QrCodeIcon className="w-6 h-6 text-[#331400]" /> </button> <button onClick={copyToClipboard}> <CopyIcon className="w-6 h-6 text-[#331400]" /> </button> <button onClick={shareLink}> <Share2Icon className="w-6 h-6 text-[#331400]" /> </button> <p className="text-sm text-gray-600">{formatLink(profileLink)}</p> </div> </div> </div>
